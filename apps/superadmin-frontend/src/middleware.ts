...
export async function middleware(request: NextRequest) {
  ...
  if (!response.ok) { const redirect = NextResponse.redirect(new URL("/login", request.url)); redirect.cookies.delete("auth_token"); redirect.cookies.delete("refresh_token"); redirect.cookies.delete("auth_role"); return redirect; }
...
export const config = { matcher: ["/tenants/:path*", "/appointments/:path*", "/status/:path*"] };