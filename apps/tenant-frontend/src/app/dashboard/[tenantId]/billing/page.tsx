  ...
  return (
    <DashboardShell tenantId={tenantId}>
      <div className="mx-auto max-w-5xl space-y-10">
        <div>
          <p className="text-sm font-semibold text-teal-700 uppercase tracking-wide">Subscription & Usage</p>
          <h2 className="mt-1 text-3xl font-semibold tracking-tight text-stone-900">Billing Overview</h2>
        ...
        <div className="rounded-xl border border-stone-200 bg-white p-6 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
              <div className="flex items-center gap-2">
                <span className="rounded-full bg-teal-50 border border-teal-100 px-3 py-1 text-xs font-semibold text-teal-700 uppercase tracking-wider">
                  {sub.planName} Plan
                </span>
                {sub.isActive ? (
              ...
              <h3 className="mt-3 text-xl font-semibold text-stone-900">Vapi Voice Calling Package</h3>
              <p className="text-xs text-stone-500 mt-1">Your current package limits and remaining days in billing cycle.</p>
            </div>
            <div className="sm:text-right border-t sm:border-t-0 pt-3 sm:pt-0 border-stone-100">
              <div className="flex items-center sm:justify-end gap-1.5 text-2xl font-bold text-stone-900">
                <Clock size={20} className="text-teal-700" />
                {sub.daysLeft} days
              </div>
              <p className="text-xs text-stone-500">remaining in current period</p>
            </div>
          </div>
          ...
          {sub.planName.toLowerCase() !== "unlimited" ? (
            <div className="mt-6 border-t border-stone-100 pt-5">
              ...
              <div className="mt-2.5 h-3 w-full rounded-full bg-stone-100 overflow-hidden">
                <div
            ...
            <div className="mt-6 flex items-center gap-2 text-sm text-teal-900 font-medium bg-teal-50/60 p-4 rounded-lg border border-teal-100">
        ...
        <div>
          <div className="text-center max-w-xl mx-auto mb-8">
            <h3 className="text-2xl font-semibold text-stone-900">Available Calling Packages</h3>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {plans.map((plan) => {
              ...
              return (
                <div
                  key={plan.name}
                  className={`relative flex flex-col rounded-xl border bg-white p-6 shadow-sm transition-all ${
                    plan.popular
                ...
                >
                  {plan.popular && (
                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-teal-700 px-3 py-0.5 text-xs font-semibold text-white shadow-sm">
                      Most Popular
                    </span>
                  )}
                  ...
                  <h4 className="mt-4 text-lg font-semibold text-stone-900">{plan.name} Plan</h4>
                  <p className="text-xs text-teal-700 font-medium mt-0.5">{plan.minutes}</p>

                  <div className="mt-4 flex items-baseline gap-1">
                    <span className="text-3xl font-bold tracking-tight text-stone-900">{plan.price}</span>
                    <span className="text-xs text-stone-500 font-medium">{plan.period}</span>
                  </div>

                  <ul className="mt-6 space-y-3 text-xs text-stone-600 flex-1">
        ...
        {/* Custom Plan Banner */}
        <div className="rounded-xl border border-slate-200 bg-slate-900 p-8 text-white flex flex-col md:flex-row items-center justify-between gap-6 shadow-md">
          <div className="space-y-1">
            ...
            <h4 className="text-xl font-semibold text-white">Need more than 5,000 minutes per month?</h4>
          ...
          <a
            ...
            className="shrink-0 rounded-lg bg-[#ddf070] px-5 py-3 text-sm font-semibold text-[#173a2d] hover:bg-lime-400 transition"
          >
            Contact Sales Team
          </a>
        </div>
