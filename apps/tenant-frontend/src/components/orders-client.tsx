      ...
      <div className="flex flex-col gap-4 rounded-lg border border-black/5 bg-white p-4 shadow-sm md:flex-row md:items-center">
        ...
        <div className="flex items-center gap-3">
          ...
          <select
            ...
            className="rounded-md border border-stone-200 p-2 text-sm focus:border-teal-500 focus:outline-none capitalize"
          >
            {statuses.map(status => (
        ...
        <table className="w-full min-w-220 text-left text-sm">
          ...
          <tbody className="divide-y divide-stone-100">
            {filteredOrders.map((order) => (
              <tr key={order.orderId} className="transition hover:bg-stone-50/50">
                <td className="px-5 py-4 font-mono text-xs text-stone-600">{order.orderId.substring(0, 8)}...</td>
                <td className="px-5 py-4">
                  ...
                  {order.customerAddress && <p className="text-xs text-stone-500">{order.customerAddress}</p>}
                </td>
                <td className="px-5 py-4 capitalize font-semibold text-stone-700">{order.orderType}</td>
                <td className="px-5 py-4 font-semibold text-stone-900">{formatPrice(order.totalAmount, currency)}</td>
                <td className="px-5 py-4">
                  <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-xs font-semibold capitalize ${getStatusColor(order.status)}`}>
                    {order.status}
                  </span>
                </td>
                ...
                <td className="px-5 py-4 text-right">
                  <select
                    ...
                    className="rounded-md border border-stone-200 p-1 text-xs focus:border-teal-500 focus:outline-none capitalize"
                  >
