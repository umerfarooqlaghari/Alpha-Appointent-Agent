  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-semibold text-teal-700">SERVICE CATALOG & PRICING</p>
          <h2 className="mt-1 text-3xl font-semibold tracking-tight">Services</h2>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredServices.map((service) => (
          <div
            key={service.id}
            className={`group relative rounded-xl border border-stone-200 bg-white p-5 shadow-sm transition-all hover:shadow-md flex flex-col justify-between ${
              service.isDisabled ? "opacity-60 bg-stone-50/50" : ""
            }`}
          >
            <div>
              <div className="flex justify-between items-start gap-3">
                <div>
                  <span className="inline-block rounded-full bg-teal-50 px-2.5 py-0.5 text-xs font-semibold text-teal-800 mb-2">
                    {service.category}
                  </span>
                  <h3 className="font-bold text-lg text-stone-900 leading-tight">{service.name}</h3>
                </div>
                {service.price != null && (
                  <span className="font-bold text-lg text-emerald-700 bg-emerald-50 px-2.5 py-1 rounded-lg shrink-0">
              ...
              {service.description && (
                <p className="mt-3 text-sm text-stone-500 line-clamp-3 leading-relaxed">
                  {service.description}
                </p>
              )}

              {service.durationMinutes != null && (
                <div className="mt-4 flex items-center gap-1.5 text-xs font-semibold text-stone-600">
            ...
            <div className="mt-6 flex justify-between items-center border-t border-stone-100 pt-4">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="p-1.5 text-stone-400 hover:text-teal-600 hover:bg-teal-50 rounded-md transition-colors"
                >
                  <Edit2 size={16} />
                </button>
                <button
        ...
        {filteredServices.length === 0 && (
          <div className="col-span-full rounded-xl border border-dashed border-stone-300 p-12 text-center">
            <Briefcase className="mx-auto text-stone-300 mb-3" size={48} />
            <h3 className="text-lg font-semibold text-stone-900">No services found</h3>
            <p className="mt-1 text-sm text-stone-500">Add your first service to get started.</p>
          </div>
        )}
      </div>
            ...
            <form onSubmit={handleFormSubmit} className="mt-4 space-y-4">
              <label className="block text-xs font-semibold text-stone-700">
                ...
                <input
                  ...
                  className="mt-1 w-full rounded-md border border-stone-200 p-2.5 text-sm focus:border-teal-500 focus:outline-none"
                />
              </label>
              ...
              {editingService && (
                <label className="flex items-center gap-2 text-xs font-semibold text-stone-700 pt-1">
                  <input
                    type="checkbox"
                    name="isDisabled"
                    value="true"
              ...
              <div className="flex gap-3 pt-4 border-t border-stone-100">
                <button
                  ...
                  className="w-1/2 rounded-md border border-stone-200 py-2.5 text-sm font-semibold text-stone-700 hover:bg-stone-50"
                >
                  Cancel
                </button>
                <button
                  type="submit"
                  className="w-1/2 rounded-md bg-[#12382e] py-2.5 text-sm font-semibold text-white hover:bg-emerald-900 transition-colors"
                >
